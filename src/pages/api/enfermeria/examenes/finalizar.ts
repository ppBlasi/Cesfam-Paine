...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  if (!nurse) {
    return jsonResponse(403, { error: "Debes iniciar sesion como enfermeria para finalizar." });
  }
  ...
  if (order.estado === "REALIZADO") {
    return jsonResponse(400, { error: "La orden ya fue marcada como realizada." });
  }
  ...
  try {
    ...
    return jsonResponse(200, { message: "Examen marcado como realizado." });
  } catch (error) {
    console.error("exam order finalize error", error);
    return jsonResponse(500, { error: "No pudimos actualizar el estado del examen." });
  }
};
